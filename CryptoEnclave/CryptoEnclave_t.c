...
typedef struct ms_ecall_get_ecall_number_t {
	int* ms_test;
	size_t ms_int_size;
} ms_ecall_get_ecall_number_t;
...
static sgx_status_t SGX_CDECL sgx_ecall_get_ecall_number(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_get_ecall_number_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_get_ecall_number_t* ms = SGX_CAST(ms_ecall_get_ecall_number_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	int* _tmp_test = ms->ms_test;
	size_t _tmp_int_size = ms->ms_int_size;
	size_t _len_test = _tmp_int_size * sizeof(int);
	...
	ecall_get_ecall_number(_in_test, _tmp_int_size);
	if (_in_test) {
		if (memcpy_s(_tmp_test, _len_test, _in_test, _len_test)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
