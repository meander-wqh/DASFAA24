...
bool CuckooFilter::insertImpl(const size_t index, const uint32_t fingerprint, const bool kickout, Victim &victim){
	...
	if(kickout){
		size_t rand_length = 1; // 生成一个字节的随机数
		uint8_t rand_data[rand_length];

		sgx_status_t status = sgx_read_rand(rand_data, rand_length);

		// 将随机字节转换为介于0和255之间的整数
		int random_byte = rand_data[0];
